<?php
/*
 * Этот пример я скачал с сайта Тинькофф.
 * 
 * Единственное, я немного поправил конструктор. В оригинале в конструктор параметрами передавались айди терминала и ключ.
 * Я сделал так, что конструктор сам их тянет из отдельного файла, а остальные классы даже не знают об этом
 * 
 * Вообще, документация вот тут: https://acdn.tinkoff.ru/static/documents/merchant_api_protocoI_eacq.pdf
 */
...
class TinkoffMerchantAPI
{
    ...
    {
        include_once $_SERVER['DOCUMENT_ROOT'] . '/backend/core/tinkoff/tokens.php';

        $this->api_url = 'https://securepay.tinkoff.ru/v2/';
        $this->terminalKey = $tinkoffTokens['terminalKey'];
        $this->secretKey = $tinkoffTokens['secretKey'];
    }
